									...
									<FORM id="pageForm" name="customerForm"
										...
										<TABLE cellSpacing=0 cellPadding=2 border=0>
											<TBODY>
												<TR>
													<TD>客户名称：</TD>
													<TD><input type="hidden" name="customer.cust_id"
														id="cust_id" value="${param['customer.cust_id']}" /> <INPUT
														class=textbox style="WIDTH: 80px" maxLength=50
														name="cust_name" id="cust_name"
														value="${param['cust_name']}"> <input
										...
										<TBODY>
											...
											<s:iterator value="#pageBean.list">
												<TR
													...
													<TD>
														<!-- 没有传递参数,显示删除和修改操作 --> <a
														href="${pageContext.request.contextPath }/SaleVisitAction_toEdit?visit_id=<s:property value="visit_id" />">修改</a>
														&nbsp;&nbsp; <a href="javascript:void(0)"
														onclick="deleteConfirm('<s:property value="cust_name" />','${pageContext.request.contextPath }/CustomerAction_delete?cust_id=<s:property value="cust_id" />');">删除</a>
													</TD>
												</TR>
											</s:iterator>
								...
								<TD><SPAN id=pagelink>
										<DIV
											...
											到 <input type="text" size="3" id="page" name="page"
												value="<s:property value="#pageBean.currentPage" />" /> 页
